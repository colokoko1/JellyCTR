...
#define MVD_BUFFER_SIZE (1024 * 1024 * 4) 
...
void tile_linear_rgb565(u16* linear_data, u16* tiled_data) {
    // FIXED: Loop height matches the 224px 16:9 output buffer so we don't over-read into garbage memory
    for (u32 y = 0; y < 224; y++) {
        for (u32 x = 0; x < 400; x++) {
            tiled_data[get_tiled_offset(x, y, 512)] = linear_data[y * 400 + x];
        }
    }
    GSPGPU_FlushDataCache(tiled_data, 512 * 256 * 2);
}
...
void process_nal_unit(u8* data, size_t size) {
    ...
    Result res = mvdstdProcessVideoFrame(data, size, 0, &out);
    
    if (res == MVD_STATUS_FRAMEREADY) {
        mvdstdRenderVideoFrame(&mvd_config, true); 
        
        if (mvd_out_buffer && video_tex.data) {
            GSPGPU_InvalidateDataCache(mvd_out_buffer, 400 * 240 * 2);
            tile_linear_rgb565((u16*)mvd_out_buffer, (u16*)video_tex.data);
            frames_decoded++;
        }
    }
}
    ...
    for (size_t i = 0; i < total; i++) {
        ...
        if (ts_fragment_pos == TS_PACKET_SIZE) {
            if (ts_fragment[0] == 0x47) {
                ...
                int pusi = (ts_fragment[1] & 0x40) >> 6; int afc = (ts_fragment[3] & 0x30) >> 4;
                int po = 4; if (afc == 2 || afc == 3) po += 1 + ts_fragment[4];
                if ((afc == 1 || afc == 3) && po < TS_PACKET_SIZE) {
                    if (pusi && po + 3 < TS_PACKET_SIZE) {
                        if (ts_fragment[po] == 0x00 && ts_fragment[po+1] == 0x00 && ts_fragment[po+2] == 0x01) {
                            u8 s_id = ts_fragment[po+3];
                            if ((s_id & 0xF0) == 0xE0) {
                                video_pid = pid; 
                                if (ts_fragment[po+7] & 0x80) { 
                                    uint64_t pts = ((uint64_t)(ts_fragment[po+9] & 0x0E) << 29) | ((uint64_t)(ts_fragment[po+10]) << 22) | ((uint64_t)(ts_fragment[po+11] & 0xFE) << 14) | ((uint64_t)(ts_fragment[po+12]) << 7) | ((uint64_t)(ts_fragment[po+13] & 0xFE) >> 1);
                                    current_video_pts = (double)pts / 90000.0;
                                    if (start_video_pts < 0.0) start_video_pts = current_video_pts;
                                }
                                po += 9 + ts_fragment[po+8];
                            } 
                            else if ((s_id & 0xE0) == 0xC0 || s_id == 0xBD) {
                                audio_pid = pid; po += 9 + ts_fragment[po+8];
                            }
                        }
                    }
                    if (video_pid == pid && po < TS_PACKET_SIZE) {
                        size_t ps = TS_PACKET_SIZE - po;
                        if (nal_buffer_pos + ps < MVD_BUFFER_SIZE) {
                            memcpy(nal_buffer + nal_buffer_pos, ts_fragment + po, ps); 
                            nal_buffer_pos += ps;
                            
                            while (nal_buffer_pos >= 6) {
                                int s_idx = -1; 
                                for (size_t j = 0; j <= nal_buffer_pos - 3; j++) { 
                                    if (nal_buffer[j] == 0x00 && nal_buffer[j+1] == 0x00 && nal_buffer[j+2] == 0x01) { s_idx = j; break; } 
                                }
                                if (s_idx > 0) { 
                                    memmove(nal_buffer, nal_buffer + s_idx, nal_buffer_pos - s_idx); 
                                    nal_buffer_pos -= s_idx; 
                                    s_idx = 0; 
                                }
                                if (s_idx == 0) {
                                    int n_idx = -1; 
                                    for (size_t j = 3; j <= nal_buffer_pos - 3; j++) { 
                                        if (nal_buffer[j] == 0x00 && nal_buffer[j+1] == 0x00 && nal_buffer[j+2] == 0x01) { 
                                            n_idx = j; 
                                            if (j > 0 && nal_buffer[j-1] == 0x00) n_idx = j - 1; 
                                            break; 
                                        } 
                                    }
                                    if (n_idx > 0) { 
                                        GSPGPU_FlushDataCache(nal_buffer, n_idx);
                                        process_nal_unit(nal_buffer, n_idx); 
                                        memmove(nal_buffer, nal_buffer + n_idx, nal_buffer_pos - n_idx); 
                                        nal_buffer_pos -= n_idx; 
                                    } else {
                                        break; 
                                    }
                                } else {
                    ...
                    else if (audio_pid == pid && po < TS_PACKET_SIZE) {
                        size_t ps = TS_PACKET_SIZE - po;
                        for (size_t j = 0; j < ps; j++) {
                            ...
                            if (!thread_run) break;
                            audio_ptr[write_node * AUDIO_BUF_SIZE + buf_pos] = ts_fragment[po + j];
                            buf_pos++;
...
void stop_playback() {
    ...
    if (mvd_out_buffer != NULL) { linearFree(mvd_out_buffer); mvd_out_buffer = NULL; }
    if (video_tex.data != NULL) { C3D_TexDelete(&video_tex); video_tex.data = NULL; }
    
    ndspChnReset(0); start_video_pts = -1.0;
}
...
void play_current_queue_item() {
    ...
    if (item.IsVideo) {
        ...
        mvdstdGenerateDefaultConfig(&mvd_config, 400, 224, 400, 224, NULL, mvd_out_buffer, NULL);
        MVDSTD_SetConfig(&mvd_config); 

        // FIXED: UV perfectly maps the 400x224 frame inside the 512x256 texture buffer
        static Tex3DS_SubTexture v_sub = { 400, 224, 0.0f, 1.0f, 400.0f/512.0f, 1.0f - (224.0f/256.0f) };
        video_image.tex = &video_tex; video_image.subtex = &v_sub;
    ...
    curl_easy_cleanup(img_c); free(d.b); total_samples_played = 0; write_node = 0; buf_pos = 0;
    ndspChnInitParams(0); ndspChnSetFormat(0, NDSP_FORMAT_STEREO_PCM16); ndspChnSetRate(0, HARDWARE_RATE);
    for(int i=0; i<NUM_BUFFERS; i++) waveBuf[i].status = NDSP_WBUF_FREE;
    thread_run = true; pthread_create(&play_thread, NULL, [](void* arg) -> void* {
        CURL *curl = curl_easy_init(); char s[2048]; snprintf(s, 2048, "%s/Audio/%s/stream?static=false&container=wav&audioCodec=pcm_s16le&audioSampleRate=48000&maxAudioSampleRate=48000&audioChannels=2&maxAudioChannels=2&audioBitRate=1536000&api_key=%s", server_url, current_song_id, access_token);
...
int main(int argc, char* argv[]) {
    ...
    // FIXED: Ask the MVD hardware converter to natively swap the BGR/RGB channels for us
    if (R_SUCCEEDED(mvdstdInit(MVDMODE_VIDEOPROCESSING, MVD_INPUT_H264, MVD_OUTPUT_BGR565, MVD_DEFAULT_WORKBUF_SIZE, NULL))) mvd_available = true;
    ...
    while (aptMainLoop()) {
        ...
        if (current_state == STATE_VIDEO_PLAYER) { 
            if (frames_decoded > 0) { 
                // FIXED: Draw at Y=8 to beautifully letterbox the 16:9 video onto the 5:3 screen
                C2D_DrawImageAt(video_image, 0, 8, 0.5f, NULL, 1.0f, 1.0f); 
            } 
        } else { 
            if (has_album_art) C2D_DrawImageAt(album_art_image, 15, 30, 0.5f, NULL, 0.65f, 0.65f);
            C2D_Text t1; C2D_TextParse(&t1, g_dynamicBuf, is_playing ? current_song_name : "JellyCTR Ready"); C2D_DrawText(&t1, C2D_WithColor, 200, 40, 0.5f, 0.65f, 0.65f, CLR_WHITE); 
        }
    ...
    stop_playback(); 
    if (mvd_available) mvdstdExit(); 
    if (sprite_sheet) C2D_SpriteSheetFree(sprite_sheet);
    linearFree(audio_ptr); 
    linearFree(nal_buffer); 
    socExit(); 
    ndspExit(); 
    C2D_Fini(); 
    C3D_Fini(); 
